...
document.addEventListener(
	...
	async (e) => {
		...
		const question = await icp_backend.getQuestion();
		document.getElementById("question").innerText = question;

		const voteCounts = await icp_backend.getVotes();
...
pollForm.addEventListener(
	...
	async (e) => {
		...
		const updatedVoteCounts = await icp_backend.vote(checkedValue);
